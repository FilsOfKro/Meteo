...
public class Inter {
  /**********************************************************************
   * zyGrib: meteorological GRIB file viewer Copyright (C) 2008-2012 - Jacques
   * Zaninetti - http://www.zygrib.org
   * 
  ...
  // #include"DataDefines.h"#include"DataMeteoAbstract.h"

  double xmin, xmax, ymin, ymax;
  boolean duplicated;
  boolean entireWorldInLongitude;
   ...
   */
  boolean isXInMap(double x) {
    return entireWorldInLongitude || (x >= xmin && x <= xmax);
  }

  boolean isYInMap(double y) {
    return y >= ymin && y <= ymax;
  }

  boolean isPointInMap(double x, double y) {
  ...
  double getAveragePointsDensity() {
    return 0;
  }
  ...
  double getValueOnRegularGrid(int i, int j) {
    return getValue(i, j);
  }
  ...
  // =====================================================================
  double getInterpolatedValueUsingRegularGrid(double px, double py, boolean interpolateValues) {
    double val;
    ...
    if (!isOk() || getDeltaX() == 0 || getDeltaY() == 0) {
      throw new GRIBNotDefinedException();
    }
    if (!isYInMap(py)) {
      throw new GRIBNotDefinedException();
    }
    ...
    // printf("GriddedRecord::getInterpolatedValueUsingRegularGrid : %d: %f %f %f
